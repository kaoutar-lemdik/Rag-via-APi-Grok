...
import time
from chat_rag_groq import get_rag_response, init_chatbot
...
def page_chatbot():
    ...
    # ---- CHARGEMENT PINECONE UNE SEULE FOIS ----
    if not st.session_state.chatbot_ready:
        with st.spinner("⏳ Connexion à la base de connaissances..."):
            try:
                init_chatbot()
...
def page_merci():
    ...
    st.code(
        "https://rag-via-api-grok-3qs88yvxyufljb9zbhrndk.streamlit.app/",
        language=None
    )
